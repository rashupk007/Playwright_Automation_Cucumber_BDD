...
Given('I am on the login page', async function () {
    ...
    await this.loginPage.launchApplication(testData.url)
});

When('I enter valid username and password', async function () {
    await this.loginPage.validLogin(testData.username, testData.password)
});

When('I enter valid username and invalid password', async function () {
    await this.loginPage.validLogin(testData.username, testData.invalidPassword)
});

When('I click Login button with blank username and password', async function () {
    await this.loginPage.validLogin(testData.invalidUsername5, testData.invalidPassword1)
});

When('I enter valid username and click Login button with blank password', async function () {
    await this.loginPage.validLogin(testData.username, testData.invalidPassword1)
});

When('I enter invalid username without \'@gmail.com\'', async function () {
    await this.loginPage.validLogin(testData.invalidUsername, testData.password)
});

When('I enter invalid username without \'com\'', async function () {
    await this.loginPage.validLogin(testData.invalidUsername1, testData.password)
});
